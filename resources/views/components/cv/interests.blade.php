<div class="mt-6">
    <div class="flex items-center justify-between group mb-2">
        ...
        <button @click="startCreating('interest')" class="opacity-0 group-hover:opacity-100 focus-within:opacity-100 focus:opacity-100 transition-opacity text-indigo-600 hover:text-indigo-800" aria-label="Ajouter un centre d'intérêt" title="Ajouter un centre d'intérêt">
            ...
            <div class="relative group" :class="(interest.status === 'draft' || interest.proposed_action) ? 'cv-item-draft' : ''">
                ...
                <template x-if="editingItem.id === interest.id && editingItem.type === 'interest'">
                    <div class="flex items-center gap-1 bg-white p-1 rounded shadow-sm border border-indigo-100">
                        ...
                        <button @click="deleteItem('interest', interest.id)" class="p-1 text-red-400 hover:text-red-600" title="Supprimer" aria-label="Supprimer le centre d'intérêt">
