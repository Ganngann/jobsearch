<div class="flex items-center gap-2 mt-2 mb-2 group">
    ...
    <button @click="startCreating('skill')" aria-label="Ajouter une compétence" title="Ajouter une compétence" class="opacity-0 group-hover:opacity-100 focus-within:opacity-100 focus:opacity-100 transition-opacity text-indigo-600 hover:text-indigo-800">
            ...
            <template x-if="editingItem.id !== skill.id || editingItem.type !== 'skill'">
                <div class="relative">
                    ...
                    <button @click="deleteItem('skill', skill.id)" 
                            class="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full w-3.5 h-3.5 flex items-center justify-center opacity-0 group-hover:opacity-100 focus-within:opacity-100 focus:opacity-100 transition-opacity hover:bg-red-600 shadow-sm">
