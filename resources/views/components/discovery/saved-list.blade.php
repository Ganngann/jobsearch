<div class="lg:col-span-2 bg-white rounded-3xl p-6 shadow-xl border border-slate-100 flex flex-col min-h-[200px]">
    <div class="flex items-center justify-between mb-6">
        ...
        <span class="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-[10px] font-black uppercase tracking-widest" x-text="savedMetiers.length + ' sélectionnés'"></span>
    </div>

    <div class="flex flex-wrap gap-3">
        <template x-for="item in savedMetiers" :key="item.type + '-' + (item.id || item.code)">
            <div 
        ...
        <div x-show="savedMetiers.length === 0" class="w-full py-8 text-center border-2 border-dashed border-slate-100 rounded-2xl text-slate-400 text-sm italic">
