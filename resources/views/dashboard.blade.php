        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1 font-medium">
                    {{ $matches->total() }} offre(s) correspondent à votre profil <span class="text-indigo-600">#{{ Auth::user()->name }}</span>
                </p>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if($matches->isEmpty())
                <div class="bg-white rounded-3xl border-2 border-dashed border-slate-200 p-20 text-center">
                    ...
                    <p class="mt-2 text-slate-500 max-w-xs mx-auto">Lancez une synchronisation ou élargissez vos critères de recherche dans votre profil.</p>
                    <div class="mt-8">
                        <button class="px-6 py-3 bg-indigo-600 text-white font-bold rounded-2xl shadow-lg shadow-indigo-100 hover:bg-indigo-500 transition-all">
                    ...
                    @foreach($matches as $match)
                        <div class="group relative bg-white border border-slate-200 rounded-3xl p-1 transition-all duration-300 hover:shadow-2xl hover:shadow-slate-200 hover:-translate-y-1">
                            <div class="p-6">
                                <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                                    ...
                                    <div class="flex items-center gap-6 flex-1">
                                        <!-- Logo Container -->
                                        <div class="shrink-0">
                                            @if($match->jobOffer->employer->logo_base64 && strlen($match->jobOffer->employer->logo_base64) > 100)
                                                <div class="w-20 h-20 rounded-2xl bg-slate-50 border border-slate-100 p-3 flex items-center justify-center">
                                                    <img src="data:{{ $match->jobOffer->employer->logo_mime_type }};base64,{{ $match->jobOffer->employer->logo_base64 }}" class="w-full h-full object-contain" alt="Logo">
                                                </div>
                                            @else
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2 mb-2">
                                                <span class="text-[10px] font-black uppercase tracking-widest text-indigo-500">{{ $match->jobOffer->employer->label }}</span>
                                                <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                                <span class="text-[10px] font-bold uppercase tracking-widest text-slate-400">{{ $match->jobOffer->location }}</span>
                                            </div>
                                            <h3 class="text-xl font-black text-slate-900 leading-tight group-hover:text-indigo-600 transition-colors">
                                                <a href="{{ route('jobs.show', $match->jobOffer) }}">
                                    ...
                                    <div class="flex items-center gap-8 pl-6 md:border-l border-slate-100">
                                        <div class="text-center">
                                            <p class="text-4xl font-black {{ $match->final_score >= 70 ? 'text-emerald-500' : ($match->final_score >= 40 ? 'text-amber-500' : 'text-slate-400') }}">
                                                {{ $match->final_score ?? $match->pre_score }}<span class="text-lg">%</span>
                                            </p>
                                            <p class="text-[9px] font-black uppercase tracking-widest text-slate-400 mt-1">Match Score</p>
                                        </div>
                                        
                                        <a href="{{ route('jobs.show', $match->jobOffer) }}" class="p-4 rounded-2xl bg-slate-900 text-white hover:bg-indigo-600 transition-all duration-300 shadow-xl shadow-slate-200 hover:shadow-indigo-200">
                            ...
                            <div class="absolute bottom-0 left-6 right-6 h-0.5 bg-slate-50 rounded-full overflow-hidden">
                                <div class="h-full {{ $match->final_score >= 70 ? 'bg-emerald-500' : ($match->final_score >= 40 ? 'bg-amber-500' : 'bg-slate-300') }}" style="width: {{ $match->final_score ?? $match->pre_score }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
