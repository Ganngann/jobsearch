        ...
        */
        ...
        $topMetiers = Cache::remember('dashboard.top_metiers', 3600, function() {
            return \App\Models\Metier::whereHas('jobOffers')
                // ⚡ Bolt: Use targeted select before withCount to prevent memory bloat
                // 📊 Impact: Significantly reduces memory usage and cache payload size
                ->select(['id', 'label'])
                ->withCount('jobOffers')
        ...
        $topEmployers = Cache::remember('dashboard.top_employers', 3600, function() {
            return \App\Models\Employer::whereHas('jobOffers')
                // ⚡ Bolt: Use targeted select before withCount to prevent loading massive logo_base64 fields into memory and cache
                // 📊 Impact: Prevents severe memory bloat and reduces cache size drastically
                ->select(['id', 'label'])
                ->withCount('jobOffers')
                ->orderBy('job_offers_count', 'desc')
                ->limit(50)
