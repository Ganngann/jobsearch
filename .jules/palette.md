...
## 2024-05-23 - Keyboard accessibility for hover-only inline actions
**Learning:** Interactive actions hidden behind `opacity-0 group-hover:opacity-100` become completely inaccessible via keyboard tabbing because they remain invisible when focused.
**Action:** Always pair `group-hover:opacity-100` with `focus-within:opacity-100` on the parent container, and add clear `focus-visible` styles to the individual buttons.
