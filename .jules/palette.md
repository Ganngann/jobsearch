...
## 2026-06-10 - Add keyboard focus state for opacity-0 group-hover:opacity-100
**Learning:** When revealing hidden elements on hover using Tailwind's `opacity-0 group-hover:opacity-100`, elements remain invisible to keyboard users (Tabbing) unless equivalent focus states are added.
**Action:** Always pair `group-hover:opacity-100` with `group-focus-within:opacity-100` (on the parent group) or `focus:opacity-100` / `focus-within:opacity-100`.
