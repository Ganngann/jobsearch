...
## 2024-05-18 - Missing Screen Reader Context on Alpine Stateful Icons
**Learning:** Alpine.js stateful icon buttons (e.g., favorite toggles) frequently lack dynamic `aria-label` and `title` attributes that update with the component's state, leaving screen reader users without context of the current action.
**Action:** Always dynamically bind `:aria-label` and `:title` to stateful icon-only buttons using Alpine expressions to reflect their active action.
