...
## 2026-07-27 - Boolean ARIA attributes in Alpine.js
**Learning:** When dynamically binding boolean ARIA attributes in Alpine.js that must remain in the DOM when false (like `aria-expanded` or `aria-pressed`), Alpine evaluates the false state by removing the attribute entirely if it's just a boolean false, which breaks screen reader context.
**Action:** Use `.toString()` (e.g., `:aria-expanded="open.toString()"`) to ensure the attribute is rendered as a string ('true' or 'false') and remains in the DOM.
