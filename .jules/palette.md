...
## 2026-05-22 - Add ARIA Labels to Icon-Only Buttons
**Learning:** Icon-only buttons (containing only SVGs) used for actions like 'Add' or 'Delete' across CV components lack context for screen reader users without `aria-label` attributes.
**Action:** When creating or modifying icon-only buttons, especially in reusable components or lists (like CV builders), always add a descriptive `aria-label` attribute in the app's primary language to ensure they are fully accessible.
