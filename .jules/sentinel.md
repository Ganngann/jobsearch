...
## 2026-07-14 - Fix CSRF vulnerability on vector-sync route
**Vulnerability:** State-changing route /vector-sync allowed GET requests via Route::match, bypassing CSRF validation.
**Learning:** GET requests bypass the VerifyCsrfToken middleware, making state-changing operations vulnerable to CSRF.
**Prevention:** Strictly use POST, PUT, PATCH, or DELETE methods for any route that modifies application state.
